...
async function playlistRoute(req, res, course, number, playlist) {
  ...
  let openLectures = _.uniq(
    openOrders.map(order => {
      let lecture = lectures.find(
        l => l.courseLetters == order.course && l.number == order.number
      );
  ...
  if (req.user && req.user.email) {
    lectureData = openLectures.find(
      l => l.course == course && l.number == number
    );
  }

  let pageTitle = null;
  const currentLectureDataForTitle = openLectures.find(
    l => l.course == course && l.number == number
  );
  ...
  var contents = ejs.render(
    fs.readFileSync("./templates/playlist.ejs", "UTF-8"),
    {
      user: req.user,
      openLectures,
      playlistItem,
