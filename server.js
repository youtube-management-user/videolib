...
http.createServer(async function (req, res) {
  ...
  if (route == 'playlist' || route == 'video') {
    ...
    if (route == 'playlist') {
      if (currentOrder && (parseInt(lecturesMapping.indexOf(currentOrder.course)+1) == parseInt(course) && parseInt(currentOrder.number) == parseInt(number))) {
  ...
  if (route == 'playlist') {
    ...
    googleAuthRoute(req, res, query);
      ...
      if (!playlist[filename]) {
        ...
        function onError() {
          log.error(`Cannot retrieve video ${playlist[filename].url}, should be unlisted or public`);
          res.writeHead(404);
