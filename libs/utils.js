  ...
  for (var i = 1; i <= 5; i++) {
    ...
    if (fs.existsSync(path)) {
      ...
      youtubeIds.forEach((id, ind) => {
        let [youtube, local, medium, low] = id.split("|");
        if (youtube) {
          youtube = youtube.replace(/\s/, "");
        }
        const type =
          youtube == "" && (local != "" || medium != "" || low != "")
            ? {
                type: "local",
                path: local ? "./videos/" + local : null,
                medium: medium ? "./videos/" + medium : null,
                low: low ? "./videos/" + low : null
              }
            : { type: "youtube", url: youtube };
        const paddedNum = ind + 1 < 10 ? `0${ind + 1}` : ind + 1;
        playlist[
          `${coursesLatinMapping[lecturesMapping[i - 1]]}-${paddedNum}`
        ] = type;
      });
    }
  }
...
async function getOpenOrders(email) {
  function emailInGroup(groups, email, statuses) {
    ...
    if (!groups || !groups[0]) {
      console.error("emailInGroup group cache is empty");
      return false;
    }
    if (!email) {
  ...
  if (
    !fs.existsSync("./txt/orders.json") ||
    !fs.existsSync("./txt/groups.json")
  ) {
    try {
    ...
    try {
      ...
      openOrders = orders.filter(order => {
        const isTime =
          new Date(order.begin) <= new Date() &&
          new Date() <= new Date(order.end);
        const isPersonalOrder =
          order.gmail &&
          email &&
          order.gmail.toLowerCase().trim() == email.toLowerCase().trim() &&
          (parseInt(order.okl) === 1 ||
            parseInt(order.okl) === 2 ||
            parseInt(order.okl) === 6);
        const isOpenForEveryone = parseInt(order.okl) === 4;
        const isOpenForGroup =
          order.name.indexOf("group") == 0 &&
          (parseInt(order.okl) === 1 || parseInt(order.okl) === 2) &&
          emailInGroup(
            groups.filter(group => group.title == order.name),
            email,
            order.nameo
          );
        //        console.log(isTime, order.gmail, order.gmail.toLowerCase(), email.toLowerCase())
        //        if (isTime) console.log(order.nameo.indexOf('group') == 0)
        return (
          (isPersonalOrder || isOpenForEveryone || isOpenForGroup) && isTime
        );
      });
...
async function syncPaidFileStatuses() {
  ...
  const domain = "https://velikanov.ru";

  try {
    const response = await fetch(domain);
  } catch (ex) {
    console.error(
      `Domain ${domain} is unreachable, cannot sync orders status ${ex}`
    );
    return;
  }
...
function convertGroupFiles() {
  ...
  files.forEach(path => {
    ...
    groups.push({
      title: title,
      members: csv(data, ["num", "lastname", "firstname", "email"])
    });
    //    data.map(l => l.split('|'))
    //    groups.push();
  });
...
function getOrderByURL(orders, url) {
  ...
  if (orders && url && url != "" && url.indexOf("/") >= 0) {
    ...
    return orders.find(order => {
      return (
        order.course == coursesLatinMapping[course] &&
        parseInt(order.number) == parseInt(number)
      );
    });
  } else {
    return null;
